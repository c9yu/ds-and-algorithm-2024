...
for name in groupList[1:]: # 레드벨벳부터 끝까지
    ...
    curr = root
    while True:                                                ### 여러번 다시보자 진짜로!!!!!!!!!!!!!!
        if name < curr.data: # 왼쪽 트리로
            if curr.left == None: # 왼쪽에 트리가 구성되어있지 않으면
                curr.left = node # 레드벨벳가 블랙핑크 왼쪽 트리로 붙음
                break # wwhile 문 탈출
            else:
                curr = curr.left # 마마무가 레드벨벳 오쪽 트리로 붙음
        else: # 오른쪽 트리로
            if curr.right == None:
                curr.right = node # 마마무가 레드벨벳 오른쪽 트리로 붙음
                break
            else:
                curr = curr.right
...
while True:
    if deleteName == curr.data: # 삭제할 데이터 찾았으면
        ...
        elif curr.left != None and curr.right == None: # 내 노드 왼쪽에 자식노드가 있으면
